...
def onnx_simplify(onnx_dir: str, input_names, input_tensors, force=False):
    ...
    if HAS_ONNXSIM:
        ...
        # Use onnxslim instead
        model_opt = onnx.load(onnx_dir)
        model_opt = onnxslim.slim(model_opt)
        onnx.save(model_opt, sim_onnx_dir)
...
def main(args):
    ...
    # criterion and postprocessors not needed for export
    criterion, postprocessors = None, None
    ...
    # Use None to create dummy image for export (infer_dir is a directory, not an image file)
    input_tensors = make_infer_image(None, args.shape, args.batch_size, device)
    input_names = ["input"]
    output_names = ["features"] if args.backbone_only else ["dets", "labels"]

    # Use static batch size (dynamic axes cause export failures with RF-DETR's transformer architecture)
    dynamic_axes = None
    ...
    output_dir = Path(args.infer_dir)  # Export to infer dir
