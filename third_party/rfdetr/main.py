...
def get_args_parser():
    ...
    parser_export.add_argument('--batch_size', type=int, default=1, help="Batch size for export (default: 1)")
    ...
    # Add required arguments for export
    parser_export.add_argument('--device', default='cpu', help='device to use for export')
    parser_export.add_argument('--seed', default=42, type=int, help='random seed')
    return parser
...
if __name__ == '__main__':
    ...
    if args.subcommand == 'distill':
        ...
        # Save export-specific args before we modify anything
        export_args = {
            'infer_dir': args.infer_dir,
            'shape': args.shape,
            'batch_size': getattr(args, 'batch_size', 1),  # Add batch_size to export args
        ...
        # Load training config and create fresh args with all defaults
        config_path = Path(export_args['infer_dir']) / 'config.json'
        ...
        # Create new args using populate_args with training config
        full_args = populate_args(**training_config)

        # Override with export-specific args
        for key, value in export_args.items():
            setattr(full_args, key, value)

        # Set export-specific overrides (batch_size now comes from export_args)
        full_args.device = 'cpu'
        full_args.subcommand = 'export_model'

        # Set resume path to load checkpoint from infer_dir
        infer_path = Path(export_args['infer_dir'])
        if not full_args.resume:
            # Look for best checkpoint
            checkpoint_candidates = [
                infer_path / 'checkpoint_best_total.pth',  # Best of EMA and regular
                infer_path / 'checkpoint_best_regular.pth',
                infer_path / 'checkpoint_best_ema.pth',
                infer_path / 'checkpoint.pth',
            ]
            for ckpt_path in checkpoint_candidates:
                if ckpt_path.exists():
                    full_args.resume = str(ckpt_path)
                    print(f"Using checkpoint: {ckpt_path}")
                    break

        # Load config from checkpoint if available
        if full_args.resume and Path(full_args.resume).exists():
            import torch
            checkpoint = torch.load(full_args.resume, map_location='cpu', weights_only=False)
            if 'args' in checkpoint:
                # Checkpoint contains args, use those
                checkpoint_args = checkpoint['args']
                for key, value in vars(checkpoint_args).items():
                    # Don't override export-specific args or resume path
                    if key not in export_args and key != 'resume':
                        setattr(full_args, key, value)

        # Ensure all newer/optional parameters exist with defaults
        defaults = {
            'gradient_checkpointing': False,
            'patch_size': 16,
            'num_windows': 0,
        ...
        for key, default_value in defaults.items():
            if not hasattr(full_args, key):
                setattr(full_args, key, default_value)

        export_main(full_args)
