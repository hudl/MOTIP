...
echo "=== Launching RF-DETR crossing finetune (4x GPU, from ckpt5) ==="
accelerate launch --num_processes=4 train.py \
  --data-root "$SM_CHANNEL_TRAIN" \
  --exp-name rfdetr_crossing_finetune_v1 \
  --config-path ./configs/finetune_crossing_rfdetr_v1.yaml \
  --resume-model "$SM_CHANNEL_PRETRAIN/checkpoint_5.pth" \
  --resume-optimizer false \
  --resume-scheduler false \
  --num-workers 12 \
  --prefetch-factor 6 \
