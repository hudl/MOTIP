#!/bin/bash
set -euo pipefail
echo "=== MOTIP stage-3 — STAD tracking v2, ID consolidation from stage-2 checkpoint ==="
...
accelerate launch --num_processes=4 train.py \
  ...
  --exp-name motip_amf_stad_stage3_v1 \
  --config-path ./configs/r50_deformable_detr_motip_amf_stad_stage3.yaml \
