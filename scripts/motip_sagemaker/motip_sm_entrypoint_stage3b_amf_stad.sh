#!/bin/bash
set -euo pipefail
echo "=== MOTIP stage-3b — STAD, crossing/fast-mover finetuning from stage-2 checkpoint ==="
...
echo "=== Launching training (4x GPU, 3 epochs, from stage-2 checkpoint) ==="
accelerate launch --num_processes=4 train.py \
  --data-root "$SM_CHANNEL_TRAIN" \
  --exp-name motip_amf_stad_stage3b_v1 \
  --config-path ./configs/r50_deformable_detr_motip_amf_stad_stage3b.yaml \
  --resume-model "$SM_CHANNEL_PRETRAIN/checkpoint_13.pth" \
  --num-workers 12 \
  --prefetch-factor 6 \
