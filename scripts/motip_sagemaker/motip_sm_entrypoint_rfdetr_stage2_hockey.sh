...
echo "=== Launching RF-DETR stage-2 training (4x GPU, 13 epochs) ==="
accelerate launch --num_processes=4 train.py \
  --data-root "$SM_CHANNEL_TRAIN" \
  --exp-name motip_hockey_rfdetr_stage2_v1 \
  --config-path ./configs/rfdetr_motip_hockey_stage2.yaml \
  --detr-pretrain "$SM_CHANNEL_PRETRAIN/rfdetr_best_ema.pth" \
  --num-workers 12 \
  --prefetch-factor 6 \
